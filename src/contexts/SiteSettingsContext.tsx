...
// =============================================================
// Defaults — mirror the hardcoded values in landing components
// so the page renders correctly even before Supabase responds.
// =============================================================
...
const DEFAULT_STATS_BAR: StatsBarSettings = {
...
const DEFAULT_TESTIMONIALS: TestimonialsSettings = {
  items: [
    ...
    { name: 'James Kamau',    country: 'United Kingdom', flag: '🇬🇧', initials: 'JK', color: 'bg-emerald-600', stars: 5, quote: 'Finally a platform I can trust. The KYC verification and email-based withdrawal security give me complete peace of mind. Customer support is always available and genuinely helpful. Highly recommended.' },
...
const DEFAULT_FAQ: FAQSettings = {
  ...
  items: [
    ...
    { q: 'How are my funds protected?',                      a: 'We use cold wallet storage for 95% of assets, multi-signature wallets requiring multiple approvals, email OTP verification on all withdrawals, 256-bit SSL encryption, and continuous security monitoring. User funds are held in segregated accounts completely separate from company funds.' },
    ...
    { q: 'How are withdrawals verified?',                    a: 'Every withdrawal requires email verification. When you submit a withdrawal request, a 6-digit code is sent to your registered email address. Enter that code to confirm — no authenticator app needed. This ensures only you can authorise fund movements.' },
  ...
  const refresh = async () => {
    try {
      ...
      if (!data || data.length === 0) {
        setSettings(SITE_DEFAULTS)
        return
      }

      const merged: SiteSettings = { ...SITE_DEFAULTS }
